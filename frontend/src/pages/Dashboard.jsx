...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (profileData) {
        // agar naam "User" ya null hai toh Google se lo
        ...
        if (!activePlat && profileData?.platform === "both") {
          setActivePlat("overall");
        } else if (!activePlat && profileData?.platform) {
          setActivePlat(profileData.platform);
        }
      }

      let query = supabase.from("posts").select("*").eq("user_id", user.id);
      if (activePlat && activePlat !== "overall") {
  ...
  return (
    <div style={{ ...dashboardTheme, maxWidth: "1150px", margin: "0 auto", padding: "56px 32px" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "32px" }}>
        <div>
          <p style={{ color: "var(--dim)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "2px", marginBottom: "4px" }}>
            DASHBOARD
          </p>
          ...
          <p style={{ color: "var(--dim)", fontSize: "13px", marginTop: "4px" }}>
            Here's how your{" "}
            {activePlat === "overall" ? "" : isYT ? "YouTube" : "Instagram"}{" "}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          {profile?.platform === "both" ? (
            <>
              <button onClick={() => setActivePlat("overall")}
                style={{ display: "flex", alignItems: "center", gap: "6px", padding: "7px 14px", borderRadius: "8px", border: "1px solid var(--border)", background: activePlat === "overall" ? "rgba(129,140,248,0.1)" : "transparent", color: activePlat === "overall" ? "var(--dash-accent)" : "var(--dim)", cursor: "pointer", fontSize: "12px", fontWeight: "500" }}>
                Overall
              </button>
              <button onClick={() => setActivePlat("youtube")}
                style={{ display: "flex", alignItems: "center", gap: "6px", padding: "7px 14px", borderRadius: "8px", border: "1px solid var(--border)", background: activePlat === "youtube" ? "rgba(255,0,0,0.1)" : "transparent", color: activePlat === "youtube" ? "#ff4444" : "var(--dim)", cursor: "pointer", fontSize: "12px", fontWeight: "500" }}>
                <Youtube size={14} /> YouTube
              </button>
              <button onClick={() => setActivePlat("instagram")}
                style={{ display: "flex", alignItems: "center", gap: "6px", padding: "7px 14px", borderRadius: "8px", border: "1px solid var(--border)", background: activePlat === "instagram" ? "rgba(193,53,132,0.1)" : "transparent", color: activePlat === "instagram" ? "#c13584" : "var(--dim)", cursor: "pointer", fontSize: "12px", fontWeight: "500" }}>
                <Instagram size={14} /> Instagram
              </button>
            </>
          ) : (
            <div style={{ display: "flex", alignItems: "center", gap: "6px", padding: "7px 14px", borderRadius: "8px", border: "1px solid var(--border)", fontSize: "12px", color: isYT ? "#ff4444" : "#c13584" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "24px" }}>
        {[
          ...
          <div key={stat.label} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
            <div style={{ fontSize: "11px", color: "var(--dim)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "12px" }}>
              {stat.label}
            </div>
            <div style={{ fontSize: "32px", fontWeight: "700", color: "var(--text)", lineHeight: 1 }}>
              {stat.value}
            </div>
            <div style={{ fontSize: "12px", color: "#4ade80", marginTop: "6px" }}>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
          <div style={{ marginBottom: "16px" }}>
            ...
            <div style={{ fontSize: "11px", color: "var(--dim)", marginTop: "2px" }}>
              {activePlat === "overall" ? "Content" : isYT ? "YouTube" : "Instagram"} posts over first 7 days
            </div>
          </div>
          {graphData.some((d) => d.posts > 0) ? (
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={graphData}>
                ...
                <Line type="monotone" dataKey="posts" stroke="var(--dash-accent)" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: "4px" }}>
            {Array.from({ length: 30 }, (_, i) => {
              ...
              return (
                <div key={day} style={{ width: "100%", aspectRatio: "1", borderRadius: "4px", background: hasContent ? "var(--dash-accent)" : "var(--border)", opacity: hasContent ? 1 : 0.4, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "8px", color: hasContent ? "#fff" : "var(--dim)" }}>
                  {day}
                </div>
              );
            })}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px" }}>
          {quickActions.map((action) => (
            <div key={action.title} onClick={() => navigate(action.href)}
              style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px", cursor: "pointer" }}
      ...
      <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span onClick={() => navigate("/posts")} style={{ fontSize: "12px", color: "var(--dash-accent)", cursor: "pointer" }}>View all →</span>
        </div>
        {posts.length === 0 ? (
          ...
          posts.slice(0, 5).map((post) => (
            <div key={post.id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px 20px", borderBottom: "1px solid var(--border)" }}>
              ...
              <span style={{ fontSize: "10px", padding: "3px 10px", borderRadius: "4px", background: post.status === "done" ? "rgba(74,222,128,0.08)" : post.status === "planned" ? "rgba(129,140,248,0.08)" : "var(--border)", color: post.status === "done" ? "#4ade80" : post.status === "planned" ? "var(--dash-accent)" : "var(--dim)" }}>
                {post.status}
              </span>
            </div>
          ))
