...
function DashboardIG() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: postsData } = await supabase.from("posts").select("*").eq("user_id", user.id).eq("platform", "instagram").order("day", { ascending: true });
      setPosts(postsData || []);
      const { data: planner } = await supabase.from("planner").select("*").eq("user_id", user.id).eq("platform", "instagram").order("day", { ascending: true });
    ...
    <div style={{ width: "100%", maxWidth: "1100px", margin: "0 auto", padding: "0 0 40px 0", boxSizing: "border-box" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "24px" }}>
        <div>
          ...
          <p style={{ color: "var(--dim)", fontSize: "13px", marginTop: "4px" }}>Here's how your Instagram content is performing today.</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "24px" }}>
        {[
          ...
          <div key={stat.label} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
            ...
            <div style={{ fontSize: "12px", color: "var(--dim)", marginTop: "6px" }}>Instagram</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "12px", marginBottom: "24px" }}>
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
          <div style={{ marginBottom: "16px" }}>
            ...
            <div style={{ fontSize: "11px", color: "var(--dim)", marginTop: "2px" }}>Instagram posts over first 7 days</div>
          </div>
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px" }}>
          {[
            { icon: "✦", title: "Content Generator", desc: "Generate hooks, captions, and CTAs for your next reel." },
            { icon: "▦", title: "Content Planner", desc: "Organize your Instagram content pipeline for the month." },
            { icon: "◫", title: "Posts", desc: "View and manage all your reels and post drafts." },
          ].map(action => (
      ...
      <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: "14px", fontWeight: "600", color: "var(--text)" }}>Recent Reels & Posts</span>
